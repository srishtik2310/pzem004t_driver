...
bool is_modbus_client_initialized(int iface)
{
	...
	adu.fc = 0x03;   // Function code for "Read Holding Registers"
	adu.unit_id = 1; // Dummy unit ID
	adu.length = 0;  // No data required for this test

	// Attempt to send the ADU
...
static int pzem004t_init(const struct device *dev)
{
	...
	LOG_INF("Initializing PZEM004T device with Modbus interface: %s",
		config->modbus_iface_name);
	LOG_INF("Modbus address: 0x%02X", config->modbus_address);

	// Get the Modbus interface index
	...
	// Check if the Modbus client is already initialized
	if (is_modbus_client_initialized(iface)) {
		LOG_INF("Modbus client for interface %d is already initialized", iface);
	} else {
		...
		LOG_INF("Modbus client initialized for interface %d", iface);
	}

	// Store the Modbus interface in the instance data
	data->iface = iface;
